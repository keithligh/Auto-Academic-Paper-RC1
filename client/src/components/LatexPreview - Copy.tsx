...
function sanitizeLatexForBrowser(latex: string): SanitizeResult {
  ...
  const createTikzBlock = (tikzCode: string, options: string = ''): string => {
    ...
    // NEW (v1.6.5): UNIVERSAL COORDINATE PARSER (Physical Units)
    // 1. Extract Scalars (x=..., y=...)
    // We must respect the user's explicit grid size for correct span calculation.
    const xMatch = options.match(/x\s*=\s*([\d.]+)/);
    const yMatch = options.match(/y\s*=\s*([\d.]+)/);
    const xUnit = xMatch ? parseFloat(xMatch[1]) : 1.0;
    const yUnit = yMatch ? parseFloat(yMatch[1]) : 1.0;
    const hasExplicitScale = !!(xMatch || yMatch);

    // 2. Extract Coordinates (All (x,y) pairs)
    ...
    // 3. Calculate Physical Spans (cm)
    // BUG FIX: Previous version used raw units (9.5) instead of physical size (9.5 * 0.8 = 7.6cm)
    const rawHorizontalSpan = (maxX !== -Infinity && minX !== Infinity) ? (maxX - minX) : 0;
    const rawVerticalSpan = (maxY !== -Infinity && minY !== Infinity) ? (maxY - minY) : 0;

    const physicalWidth = rawHorizontalSpan * xUnit;
    const physicalHeight = rawVerticalSpan * yUnit;

    // 4. Detect Structural Patterns
    const isWideHorizontal = physicalWidth > 14; // > 14cm exceeds A4 safe width
    const aspectRatio = physicalHeight > 0 ? physicalWidth / physicalHeight : 0;
    const isFlat = physicalWidth > 0 && physicalHeight > 0 && aspectRatio > 3.0;

    // NEW Intent: BRACE (Vertical Breathing Room)
    // Detects diagrams with bottom braces (negative Y) that need Y-boost
    // CRITICAL FIX: Must check 'tikzCode' (original) because polyfill removes 'brace' from 'safeTikz'
    const hasBraces = tikzCode.includes('brace') || tikzCode.includes('decorate');
    const isBraceDiagram = hasBraces && minY < -0.5;

    // REFACTOR (v1.6.5): HYBRID INTENT ENGINE
    // Priority: WIDE > BRACE > FLAT > Explicit Node Dist > Implicit (Text/Count)

    let intent = 'MEDIUM'; // Default
    let nodeDist = 2.0; // Default assumption
    ...
    if (isWideHorizontal) {
      intent = 'WIDE'; // Hard overflow constraint
    } else if (isBraceDiagram) {
      intent = 'BRACE'; // Vertical spacing constraint
    } else if (isFlat) {
      intent = 'FLAT'; // Aspect ratio constraint
    } else if (distMatch) {
      // Explicit Intent
    ...
    // DEBUG: Prove it's not dead code
    console.log(`[LatexPreview] TikZ Intent: ${intent}`, {
      physicalWidth,
      physicalHeight,
      hasExplicitScale,
      isBraceDiagram,
    ...
    if (intent === 'WIDE') {
      // GOAL: Shrink to fit A4 width (14cm target)
      const scaleFactor = Math.min(1.0, 14 / physicalWidth);
      const scale = Math.max(0.5, scaleFactor * 0.9);
      if (!options.includes('scale=')) extraOpts += `, scale=${scale.toFixed(2)}`;
      if (!options.includes('transform shape')) extraOpts += ', transform shape'; // Proportional shrink

    } else if (intent === 'BRACE') {
      // GOAL: Vertical Breathing Room (Y-Boost)
      // We keep X as is (or user defined), but expand Y to fit labels under the axis.
      // Typical brace labels need ~1.2cm - 1.5cm vertical clearance per unit.

      const targetYUnit = Math.max(yUnit * 1.5, 1.2); // Boost by 50% or floor at 1.2cm

      // We inject the NEW y-value. TikZ uses the last value provided.
      extraOpts += `, y=${targetYUnit.toFixed(2)}cm`;

      // Do NOT apply transform shape (keep text standard size)
      // Do NOT scale down (unless wide, but WIDE intent handles that)

    } else if (intent === 'FLAT') {
      // GOAL: Balance Aspect Ratio (Timeline)
      const targetRatio = 2.0;
      const yBoost = aspectRatio / targetRatio;
      const yMultiplier = Math.min(3.0, Math.max(1.5, yBoost));

      const newX = (xUnit * 1.5).toFixed(1); // 50% expansion for labels
      const newY = (yUnit * yMultiplier).toFixed(1);

      // Strip old values prevents conflict, though TikZ last-wins usually works.
      // Being safe:
      options = options.replace(/x\s*=\s*[\d.]+\s*(cm)?/g, '');
      options = options.replace(/y\s*=\s*[\d.]+\s*(cm)?/g, '');

      extraOpts += `, x=${newX}cm, y=${newY}cm`;

    } else if (intent === 'COMPACT') {
      // GOAL: Fit Pipeline to A4
      ...
      if (physicalWidth > 0) {
        // Expand to fill 14cm
        // Use physical width to detect if we need expansion
        // If physical width is small (e.g. 5cm), expand it.
        const expansionFactor = 14 / Math.max(physicalWidth, 5);
        optimalUnit = Math.min(2.5, xUnit * expansionFactor);
      } else {
        optimalUnit = 1.5;
      }
      ...
      // MEDIUM
      // BUG FIX: Implicit Fallback Logic
      // If user has explicit scale (x=0.8), we trust them and DO NOT shrink to MEDIUM (scale=0.9).
      // We also respect their desire for proportional text scaling if provided.

      let scale = 0.9;
      // Smart Scale: If it fits, sit.
      if (physicalWidth > 0 && physicalWidth <= 14) {
        scale = 1.0;
      } else {
      ...
      // UNLESS it's physically overflowing
      if (!options.includes('scale=') && !options.includes('x=') && !options.includes('y=')) {
        if (physicalWidth > 14) extraOpts += `, scale=0.9`; // Safety shrink
        else extraOpts += `, scale=${scale}`;
      }

      // NEW (v1.6): Intelligent Explicit Scaling
    ...
    const isSmallOrUnknown = rawHorizontalSpan === 0 || rawHorizontalSpan < 7;
