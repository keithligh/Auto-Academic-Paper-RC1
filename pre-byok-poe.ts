    ...
    async () => {
      ...
      if (parsed.sections && Array.isArray(parsed.sections)) {
        ...
        parsed.sections.forEach((section: any) => {
          // Robust sanitization (whitespace tolerant)
          section.content = section.content.replace(/\\begin\s*\{\s*thebibliography\s*\}[\s\S]*?\\end\s*\{\s*thebibliography\s*\}/gi, "");
          section.content = section.content.replace(/(\\section\*?\{\s*References\s*\}|\\textbf\{\s*References\s*\}|References)\s*(\n\s*\[\d+\][\s\S]*)+$/i, "");
      ...
      if (parsed.references && Array.isArray(parsed.references) && parsed.references.length > 0) {
        ...
        // CRITICAL FIX: We must remove ALL existing reference sections to prevent duplicates.
        // The AI might return multiple, or we might have one from a previous pass.
        // We filter them all out, then append our single, correct, compiled bibliography.
        ...
        await log(`[Compiler] Appending new References section (cleaned previous instances).`);
...
% Fonts & Typography
\\usepackage{newtxtext}  % Times-like serif (professional)
\\usepackage{newtxmath}  % Matching math font
\\usepackage{microtype}  % Better justification (fixes overfull/underfull boxes)
\\linespread{1.6}        % Comfortable reading space
...
\\setlength{\\parindent}{0pt}  % No indentation (modern style)
...
% Abstract Styling
\\usepackage{abstract}
\\renewcommand{\\abstractnamefont}{\\normalfont\\scshape\\centering}  % Small caps title
\\renewcommand{\\abstracttextfont}{\\normalfont\\small\\itshape}      % Italic text
\\setlength{\\absleftindent}{4em}   % Indent from left
\\setlength{\\absrightindent}{4em}  % Indent from right
...
`;
  ...
  for (const sec of analysis.sections) {
    ...
    // SANITIZATION: Remove bibliography blocks from non-Reference sections
    // The AI sometimes puts \begin{thebibliography} inside section content (WRONG)
    // Only the dedicated "References" section at the end should have it (CORRECT)
    const isRefSection = /^(\d+\.?\s*)?(references|bibliography|works cited)$/i.test(sec.name.trim());

    if (!isRefSection) {
      // 1. Aggressive Truncation: If we see ANY bibliography marker, cut everything after it.
      // Markers: \begin{thebibliography}, \end{thebibliography}, \bibitem
      // We use a single robust regex to find the FIRST occurrence of any of these.
      // This handles cases where the start tag is missing, malformed, or if only items are present.
      const bibMatch = secContent.match(/(\\begin\s*\{\s*thebibliography\s*\}|\\end\s*\{\s*thebibliography\s*\}|\\bibitem)/i);

      if (bibMatch && bibMatch.index !== undefined) {
        console.log(`[Sanitization] Detected bibliography marker ('${bibMatch[0]}') in '${secName}'. Truncating section.`);
        secContent = secContent.substring(0, bibMatch.index).trim();
      }

      // 2. Truncate content if a References header is found (Structural Sanitization)
      const refHeaderMatch = secContent.match(/(?:\\(sub)*section\*?|\\paragraph\*?|\\textbf)\s*\{\s*(References|Bibliography|Works Cited)\s*\}|(?:^|\n)\s*(References|Bibliography|Works Cited)\s*(?:\n|$)/i);

      if (refHeaderMatch && refHeaderMatch.index !== undefined) {
        console.log(`[Sanitization] Detected mid-section bibliography header in '${secName}'. Truncating...`);
        secContent = secContent.substring(0, refHeaderMatch.index).trim();
      }
    } else {
      // FIX: Escape unescaped ampersands in References section
      // The AI often outputs "Author A & Author B" which causes "Misplaced alignment tab character &"
      // We look for & that is NOT preceded by \
      secContent = secContent.replace(/(?<!\\)&/g, '\\&');
    }
