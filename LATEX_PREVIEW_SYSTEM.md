...
> **⚠️ IMPORTANT (v1.2.0, Updated v1.5.6)**: The order of extraction matters!
...
**Rationale**: If `\[...\]` is extracted before `align*`, and an align* environment contains a `\[...\]` block (though invalid LaTeX, sometimes generated by AI), the inner block gets extracted first, leaving placeholders inside the align* content. When KaTeX tries to render the align* environment, it sees "LATEXPREVIEWMATH0" as LaTeX code and fails. By extracting structured environments first, we preserve their integrity.
