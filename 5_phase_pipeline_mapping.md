...
| **4** | **The Editor** | Synthesis | **Writer Agent** | **SYNTHESIS:** Rewrites text to incorporate evidence. <br> **CITATIONS:** Uses `\cite{ref_X}`. <br> **SANITIZATION:** Runs `sanitizeLatexOutput` (server-side). |
| **5** | **The Compiler** | Formatting | **System** | Generates Bibliography. <br> **PREVIEW:** Uses `LatexPreview.tsx` with **Fragment Rendering** (strips preamble, handles blockquotes). |
...
*   **The Writer Agent is present:** It performs the heavy lifting in **Phase 1** (Drafting) and **Phase 4** (Synthesis).
*   **3 Agents, 5 Steps:**
